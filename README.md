## Alex Chernysh
Applied AI Systems Engineer. Agent infra, MCP, evals.

**[Bernstein](https://bernstein.run)**: open-source multi-agent orchestrator. 17+ adapters, worktree isolation per agent, janitor verification, MCP/A2A, HMAC audit trail. Deterministic Python control plane.

🇮🇱 Tel Aviv · [LinkedIn](https://www.linkedin.com/in/alex-chernysh/) · [hello@alexchernysh.com](mailto:hello@alexchernysh.com) · [alexchernysh.com](https://alexchernysh.com/)
